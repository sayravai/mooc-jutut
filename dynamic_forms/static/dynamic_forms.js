function dynamic_forms_textarea() {
  ...
  var addButton = function(symbol, text, action, style='primary') {
    var btn = $('<button class="btn btn-'+style+' btn-xs" type="button"><span class="bi bi-'+symbol+'"></span> '+text+'</button>"');
    menu.append(btn);
    btn.on('click', action);
    return btn;
  };
  ...
  var addLink = function(symbol, text, url, style='primary') {
    var btn = $('<a class="btn btn-'+style+' btn-xs" rel="button"><span class="bi bi-'+symbol+'"></span> '+text+'</a>"');
    btn.attr('href', url);
    menu.append(btn);
    return btn;
  }
